...
router.get('/calendario', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (errFollowups) throw errFollowups;

  // 3. Buscar apolices com renovacao no periodo
  const { data: renovacoes, error: errRenovacoes } = await supabase
    .from('apolices')
  ...
  if (errRenovacoes) throw errRenovacoes;
  ...
  for (const fu of followups || []) {
    eventos.push({
      ...
      cliente: (fu.cliente as any)?.nome,
      cliente_id: (fu.cliente as any)?.id,
      referencia_id: fu.id,
  ...
  for (const apolice of renovacoes || []) {
    eventos.push({
      ...
      cor: 'cyan',
      concluido: false,
      cliente: (apolice.cliente as any)?.nome,
      cliente_id: (apolice.cliente as any)?.id,
...
router.get('/calendario/dia/:data', asyncHandler(async (req: Request, res: Response) => {
...
router.get('/tarefas/stats/summary', asyncHandler(async (req: Request, res: Response) => {
  ...
  const { count: alta_prioridade } = await supabase
    .from('tarefas')
    .select('*', { count: 'exact', head: true })
    .eq('prioridade', 'alta')
    .eq('concluida', false);
...
// Listar tarefas
router.get('/tarefas', asyncHandler(async (req: Request, res: Response) => {
...
router.get('/tarefas/:id', asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
// Criar tarefa
router.post('/tarefas', asyncHandler(async (req: Request, res: Response) => {
...
// Atualizar tarefa
router.put('/tarefas/:id', asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
// Deletar tarefa
router.delete('/tarefas/:id', asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
// Toggle concluida
router.patch('/tarefas/:id/toggle', asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
