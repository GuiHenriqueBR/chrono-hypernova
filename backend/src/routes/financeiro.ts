...
// Listar comissões
router.get('/comissoes', asyncHandler(async (req: Request, res: Response) => {
...
router.get('/comissoes/:id', asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
// Criar comissão
router.post('/comissoes', asyncHandler(async (req: Request, res: Response) => {
...
// Atualizar comissão
router.put('/comissoes/:id', asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
// Deletar comissão
router.delete('/comissoes/:id', asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
router.get('/comissao-config', asyncHandler(async (req: Request, res: Response) => {
...
router.get('/comissao-config/:id', asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
router.post('/comissao-config', asyncHandler(async (req: Request, res: Response) => {
...
router.put('/comissao-config/:id', asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  const updates = req.body;
...
router.delete('/comissao-config/:id', asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
router.get('/seguradoras', asyncHandler(async (req: Request, res: Response) => {
  const { data, error } = await supabase
    .from('comissao_configuracoes')
    .select('seguradora')
    .order('seguradora', { ascending: true });
...
// Listar ramos únicos
router.get('/ramos', asyncHandler(async (req: Request, res: Response) => {
  const ramos = [
...
router.post('/calcular-comissao', asyncHandler(async (req: Request, res: Response) => {
...
router.get('/charts', asyncHandler(async (req: Request, res: Response) => {
  ...
  // Agrupar por mês
  const comissoesPorMes: Record<string, { recebido: number; pendente: number; bruto: number }> = {};
...
router.post('/recalcular-comissoes', asyncHandler(async (req: Request, res: Response) => {
  ...
  const { data: apolicesSemComissao, error: apolicesError } = await supabase
    ...
    .not('id', 'in', 
      supabase.from('comissoes').select('apolice_id')
    );
...
router.get('/exportar', asyncHandler(async (req: Request, res: Response) => {
  ...
  // Adicionar totais
  const totalBruto = comissoes?.reduce((acc: number, c: any) => acc + (c.valor_bruto || 0), 0) || 0;
  const totalLiquido = comissoes?.reduce((acc: number, c: any) => acc + (c.valor_liquido || 0), 0) || 0;
  rows.push([]);
...
router.get('/projecao', asyncHandler(async (req: Request, res: Response) => {
  ...
  const getPercentualComissao = (seguradora: string, ramo: string): number => {
    let config = configs?.find(c => c.seguradora === seguradora && c.ramo === ramo);
    if (!config) config = configs?.find(c => c.seguradora === seguradora && c.ramo === 'todos');
    if (!config) config = configs?.find(c => c.seguradora === 'Outros' && c.ramo === 'todos');
    return config?.percentual_comissao || 15; // Default 15%
  };

  // 4. Calcular projecao por mes
  const projecaoPorMes: Record<string, {
    mes: string;
    label: string;
    renovacoes: number;
