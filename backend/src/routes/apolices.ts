...
router.get('/:id', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (error) {
    logger.error('Erro ao buscar apolice:', error);
    throw error;
  }
...
router.post('/', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (coberturas && coberturas.length > 0) {
    ...
    await supabase.from('apolice_coberturas').insert(coberturasData);
  }

  // Registrar no historico
  await supabase.from('apolice_historico').insert({
    apolice_id: novaApolice.id,
    usuario_id: userId,
    tipo_evento: 'criacao',
...
router.put('/:id', asyncHandler(async (req: Request, res: Response) => {
  ...
  const { data: apoliceAnterior } = await supabase
    .from('apolices')
    .select('*')
    .eq('id', id)
    .single();
...
// Atualizar cobertura
router.put('/:id/coberturas/:coberturaId', asyncHandler(async (req: Request, res: Response) => {
...
// Remover cobertura
router.delete('/:id/coberturas/:coberturaId', asyncHandler(async (req: Request, res: Response) => {
  const { id, coberturaId } = req.params;
  const userId = (req as any).user?.id;
...
router.get('/:id/endossos', asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
// Criar endosso
router.post('/:id/endossos', asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  const userId = (req as any).user?.id;
...
router.patch('/:id/endossos/:endossoId', asyncHandler(async (req: Request, res: Response) => {
