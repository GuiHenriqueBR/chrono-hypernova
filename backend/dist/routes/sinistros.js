...
router.get('/:id', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    `)
        ...
        .single();
    if (error) {
        logger_1.logger.error('Erro ao buscar sinistro:', error);
        throw error;
    }
    if (!data) {
...
router.post('/', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    await supabase_1.supabase.from('sinistro_regulacao').insert({
        sinistro_id: data.id,
        usuario_id: userId,
        etapa: 'notificado',
...
router.put('/:id', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const { data: sinistroAnterior } = await supabase_1.supabase
        .from('sinistros')
        .select('status')
        .eq('id', id)
        .single();
...
router.get('/:id/regulacao', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const { data, error } = await supabase_1.supabase
...
router.post('/:id/regulacao', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const userId = req.user?.id;
...
router.patch('/:id/regulacao/:eventoId', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (status === 'concluido')
        updates.data_conclusao = data_conclusao || new Date().toISOString();
    const { data, error } = await supabase_1.supabase
        .from('sinistro_regulacao')
        .update(updates)
        .eq('id', eventoId)
...
// Listar documentos
router.get('/:id/documentos', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const { data, error } = await supabase_1.supabase
...
router.post('/:id/documentos', upload.single('arquivo'), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const userId = req.user?.id;
    ...
    const uploadResult = await (0, storage_1.uploadSinistroDocumento)(id, file.buffer, {
    ...
    if (error)
        throw error;
    // Adicionar evento na timeline
    await supabase_1.supabase.from('regulacao_sinistro').insert({
        sinistro_id: id,
    ...
    const signedUrl = await (0, storage_1.getSignedUrl)('documentos', uploadResult.path);
    res.status(201).json({ ...data, url_assinada: signedUrl });
}));
// Obter URL assinada para download de documento
router.get('/:id/documentos/:docId/download', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const signedUrl = await (0, storage_1.getSignedUrl)('documentos', doc.url_storage, 3600);
    if (!signedUrl) {
...
router.patch('/:id/documentos/:docId', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id, docId } = req.params;
...
router.delete('/:id/documentos/:docId', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id, docId } = req.params;
    // Buscar documento para obter path do storage
    const { data: doc } = await supabase_1.supabase
        .from('sinistro_documentos')
        .select('url_storage')
        .eq('id', docId)
        .eq('sinistro_id', id)
        .single();
    if (doc?.url_storage) {
        await (0, storage_1.deleteFile)('documentos', doc.url_storage);
    }
    const { error } = await supabase_1.supabase
        .from('sinistro_documentos')
        .delete()
...
router.get('/checklists/padrao/:ramo', (0, errorHandler_1.asyncHandler)(async (req, res) => {
...
router.get('/checklists/ramos', (0, errorHandler_1.asyncHandler)(async (req, res) => {
...
router.get('/:id/checklist', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    // Buscar sinistro com ramo da apolice
    const { data: sinistro, error: errSinistro } = await supabase_1.supabase
        .from('sinistros')
    ...
    const { data: checklist, error } = await supabase_1.supabase
    ...
    if (!checklist || checklist.length === 0) {
        const ramo = sinistro.apolices?.ramo?.toLowerCase().replace(/\s+/g, '_') || 'auto';
        ...
        const { data: checklistCriado, error: errCreate } = await supabase_1.supabase
            .from('sinistro_checklists')
            .insert(novosItens)
            .select();
        if (errCreate)
...
router.post('/:id/checklist', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    ...
    const { data: ultimoItem } = await supabase_1.supabase
        .from('sinistro_checklists')
        .select('ordem')
        .eq('sinistro_id', id)
...
router.delete('/:id/checklist/:itemId', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id, itemId } = req.params;
    const { error } = await supabase_1.supabase
        .from('sinistro_checklists')
        .delete()
...
router.post('/:id/checklist/:itemId/vincular', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id, itemId } = req.params;
    ...
    const { data: documento } = await supabase_1.supabase
        .from('sinistro_documentos')
...
router.post('/:id/checklist/resetar', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    // Buscar ramo do sinistro
    const { data: sinistro } = await supabase_1.supabase
        .from('sinistros')
    ...
    const { data: checklistCriado, error } = await supabase_1.supabase
        .from('sinistro_checklists')
        .insert(novosItens)
        .select();
    if (error)
...
router.get('/stats/summary', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { count: total } = await supabase_1.supabase
        .from('sinistros')
        .select('*', { count: 'exact', head: true });
    const { count: abertos } = await supabase_1.supabase
        .from('sinistros')
        .select('*', { count: 'exact', head: true })
        .not('status', 'in', '(pago,recusado)');
    const { count: em_regulacao } = await supabase_1.supabase
        .from('sinistros')
        .select('*', { count: 'exact', head: true })
        .eq('status', 'regulacao');
    const { count: pagos } = await supabase_1.supabase
        .from('sinistros')
        .select('*', { count: 'exact', head: true })
        .eq('status', 'pago');
    const { count: recusados } = await supabase_1.supabase
        .from('sinistros')
        .select('*', { count: 'exact', head: true })
        .eq('status', 'recusado');
    ...
    const { data: indenizacoes } = await supabase_1.supabase
        .from('sinistros')
