...
router.get('/dashboard', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const { data: comissoesMes } = await supabase_1.supabase
        .from('comissoes')
        .select('valor_liquido')
        .gte('data_receita', firstDay.toISOString().split('T')[0])
        .in('status', ['recebida', 'paga']);
    const receitaMes = comissoesMes?.reduce((acc, c) => acc + (c.valor_liquido || 0), 0) || 0;
    // Contagem de comissões pendentes
    const { count: comissoesPendentes } = await supabase_1.supabase
        .from('comissoes')
        .select('*', { count: 'exact', head: true })
        .eq('status', 'pendente');
    // Valor total pendente
    const { data: pendentesData } = await supabase_1.supabase
        .from('comissoes')
        .select('valor_liquido')
        .eq('status', 'pendente');
    const valorPendente = pendentesData?.reduce((acc, c) => acc + (c.valor_liquido || 0), 0) || 0;
    // Total recebido (histórico)
    const { data: comissoesRecebidas } = await supabase_1.supabase
        .from('comissoes')
        .select('valor_liquido')
        .in('status', ['recebida', 'paga']);
    const totalRecebido = comissoesRecebidas?.reduce((acc, c) => acc + (c.valor_liquido || 0), 0) || 0;
    res.json({
        receitaMes,
...
// Listar comissões
router.get('/comissoes', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { apolice_id, status, mes } = req.query;
...
router.get('/comissoes/:id', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
...
// Criar comissão
router.post('/comissoes', (0, errorHandler_1.asyncHandler)(async (req, res) => {
...
// Atualizar comissão
router.put('/comissoes/:id', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
...
// Deletar comissão
router.delete('/comissoes/:id', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
...
router.get('/comissao-config', (0, errorHandler_1.asyncHandler)(async (req, res) => {
...
router.get('/comissao-config/:id', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const { data, error } = await supabase_1.supabase
        .from('comissao_configuracoes')
        .select('*')
...
router.post('/comissao-config', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const config = req.body;
    // Validar campos obrigatórios
    if (!config.seguradora || !config.ramo) {
        return res.status(400).json({ error: 'Seguradora e ramo são obrigatórios' });
    }
    const { data, error } = await supabase_1.supabase
        .from('comissao_configuracoes')
        .insert([config])
    ...
    if (error) {
        if (error.code === '23505') {
            return res.status(400).json({ error: 'Já existe configuração para esta seguradora/ramo' });
        }
        throw error;
    }
    res.status(201).json(data);
...
router.put('/comissao-config/:id', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const updates = req.body;
    const { data, error } = await supabase_1.supabase
        .from('comissao_configuracoes')
...
router.delete('/comissao-config/:id', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const { error } = await supabase_1.supabase
        .from('comissao_configuracoes')
        .delete()
...
router.get('/seguradoras', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { data, error } = await supabase_1.supabase
        .from('comissao_configuracoes')
        .select('seguradora')
        .order('seguradora', { ascending: true });
...
// Listar ramos únicos
router.get('/ramos', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const ramos = [
...
router.post('/calcular-comissao', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { apolice_id } = req.body;
    if (!apolice_id) {
        return res.status(400).json({ error: 'ID da apólice é obrigatório' });
    }
    // Buscar apólice
    const { data: apolice, error: apoliceError } = await supabase_1.supabase
        .from('apolices')
        .select('*')
        .eq('id', apolice_id)
        .single();
    ...
    const { data: existingComissao } = await supabase_1.supabase
        .from('comissoes')
        .select('id')
        .eq('apolice_id', apolice_id)
        .single();
    if (existingComissao) {
        return res.status(400).json({ error: 'Já existe comissão para esta apólice' });
    }
    // Buscar configuração de comissão
    let { data: config } = await supabase_1.supabase
        .from('comissao_configuracoes')
        .select('*')
    ...
    if (!config) {
        const { data: configGenerica } = await supabase_1.supabase
            .from('comissao_configuracoes')
            .select('*')
            .eq('seguradora', apolice.seguradora)
            .eq('ramo', 'todos')
    ...
    if (!config) {
        const { data: configPadrao } = await supabase_1.supabase
            .from('comissao_configuracoes')
            .select('*')
    ...
    if (!config || config.percentual_comissao === 0) {
        return res.status(400).json({ error: 'Não há configuração de comissão para esta seguradora/ramo' });
    }
    // Calcular valores
    const valorBruto = apolice.valor_premio * (config.percentual_comissao / 100);
    ...
    // Criar comissão
    const { data: comissao, error: comissaoError } = await supabase_1.supabase
        .from('comissoes')
        .insert([{
            apolice_id,
...
router.get('/charts', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const { data: comissoesMensais } = await supabase_1.supabase
        .from('comissoes')
    ...
    const comissoesMensaisChart = Object.entries(comissoesPorMes).map(([key, values]) => ({
    ...
    const { data: comissoesPorSeguradora } = await supabase_1.supabase
        .from('comissoes')
        .select('valor_liquido, apolices(seguradora)')
        .gte('data_receita', startDate.toISOString().split('T')[0]);
    const seguradorasMap = {};
    comissoesPorSeguradora?.forEach((c) => {
        const seguradora = c.apolices?.seguradora || 'Outros';
        seguradorasMap[seguradora] = (seguradorasMap[seguradora] || 0) + (c.valor_liquido || 0);
    });
    const cores = ['#8b5cf6', '#06b6d4', '#10b981', '#f59e0b', '#ef4444', '#ec4899', '#6366f1', '#14b8a6'];
    ...
    const { data: comissoesPorRamo } = await supabase_1.supabase
        .from('comissoes')
        .select('valor_liquido, apolices(ramo)')
        .gte('data_receita', startDate.toISOString().split('T')[0]);
    const ramosMap = {};
    comissoesPorRamo?.forEach((c) => {
    ...
    const coresRamo = ['#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#06b6d4', '#ec4899', '#6366f1'];
    ...
    // 4. Totais do período
    const totalRecebido = comissoesMensais?.filter(c => c.status === 'recebida' || c.status === 'paga')
        .reduce((acc, c) => acc + (c.valor_liquido || 0), 0) || 0;
    const totalPendente = comissoesMensais?.filter(c => c.status === 'pendente')
        .reduce((acc, c) => acc + (c.valor_liquido || 0), 0) || 0;
    const totalBruto = comissoesMensais?.reduce((acc, c) => acc + (c.valor_bruto || 0), 0) || 0;
    ...
    const { data: comissoesAnterior } = await supabase_1.supabase
        .from('comissoes')
        .select('valor_liquido')
        .gte('data_receita', periodoAnteriorStart.toISOString().split('T')[0])
        .lte('data_receita', periodoAnteriorEnd.toISOString().split('T')[0])
        .in('status', ['recebida', 'paga']);
    const totalAnterior = comissoesAnterior?.reduce((acc, c) => acc + (c.valor_liquido || 0), 0) || 0;
    const variacaoPercentual = totalAnterior > 0
        ? Math.round(((totalRecebido - totalAnterior) / totalAnterior) * 100 * 100) / 100
        : 0;
    res.json({
...
router.post('/recalcular-comissoes', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    // Buscar apólices que não têm comissão
    const { data: apolicesSemComissao, error: apolicesError } = await supabase_1.supabase
        .from('apolices')
        .select('id, numero_apolice, seguradora, ramo, valor_premio, data_inicio')
        .not('id', 'in', supabase_1.supabase.from('comissoes').select('apolice_id'));
    if (apolicesError)
        throw apolicesError;
    const resultados = {
        processadas: 0,
    ...
    for (const apolice of apolicesSemComissao || []) {
        ...
        if (!config || config.percentual_comissao === 0) {
            resultados.erros.push(`Apólice ${apolice.numero_apolice}: sem configuração de comissão`);
            continue;
        }
        // Calcular valores
        const valorBruto = apolice.valor_premio * (config.percentual_comissao / 100);
        ...
        const valorLiquido = valorBruto - valorRepasse - valorImposto;
        // Criar comissão
        const { error: comissaoError } = await supabase_1.supabase
            .from('comissoes')
            .insert([{
        ...
        if (comissaoError) {
            resultados.erros.push(`Apólice ${apolice.numero_apolice}: ${comissaoError.message}`);
        }
        else {
            resultados.criadas++;
        }
...
router.get('/exportar', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const { data: comissoes, error } = await supabase_1.supabase
        .from('comissoes')
        .select(`
      id,
      valor_bruto,
    ...
    const rows = (comissoes || []).map((c) => [
        c.data_receita ? new Date(c.data_receita).toLocaleDateString('pt-BR') : '',
    ...
    // Adicionar totais
    const totalBruto = comissoes?.reduce((acc, c) => acc + (c.valor_bruto || 0), 0) || 0;
    const totalLiquido = comissoes?.reduce((acc, c) => acc + (c.valor_liquido || 0), 0) || 0;
    rows.push([]);
    rows.push(['', '', '', '', '', 'TOTAL:', '', totalBruto.toFixed(2), totalLiquido.toFixed(2), '']);
    // Converter para CSV
    const csvContent = [
        headers.join(';'),
    ...
    res.setHeader('Content-Disposition', `attachment; filename=comissoes_${periodo || 'todos'}_${new Date().toISOString().split('T')[0]}.csv`);
...
router.get('/projecao', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { meses = 6 } = req.query;
    const numMeses = Math.min(Number(meses), 12);
    const now = new Date();
    // 1. Buscar renovacoes previstas (apolices que vencem nos proximos meses)
    const dataLimite = new Date(now.getFullYear(), now.getMonth() + numMeses, 0);
    const { data: apolicesRenovar } = await supabase_1.supabase
        .from('apolices')
        .select(`
      id,
      numero_apolice,
    ...
    const { data: comissoesPendentes } = await supabase_1.supabase
        .from('comissoes')
        .select(`
      id,
      valor_liquido,
    ...
    const { data: configs } = await supabase_1.supabase
        .from('comissao_configuracoes')
        .select('*')
        .eq('ativo', true);
    // Funcao para encontrar percentual de comissao
    const getPercentualComissao = (seguradora, ramo) => {
        let config = configs?.find(c => c.seguradora === seguradora && c.ramo === ramo);
        if (!config)
            config = configs?.find(c => c.seguradora === seguradora && c.ramo === 'todos');
        if (!config)
            config = configs?.find(c => c.seguradora === 'Outros' && c.ramo === 'todos');
        return config?.percentual_comissao || 15; // Default 15%
    };
    // 4. Calcular projecao por mes
    const projecaoPorMes = {};
    // Inicializar meses
    ...
    apolicesRenovar?.forEach((apolice) => {
        ...
        if (projecaoPorMes[key]) {
            const percentual = getPercentualComissao(apolice.seguradora, apolice.ramo);
            const comissaoEstimada = (apolice.valor_premio || 0) * (percentual / 100) * 0.85; // 85% liquido
    ...
    comissoesPendentes?.forEach((comissao) => {
    ...
    Object.keys(projecaoPorMes).forEach((key) => {
        projecaoPorMes[key].totalProjetado =
            projecaoPorMes[key].comissaoEsperada + projecaoPorMes[key].comissoesPendentes;
    });
    // 5. Buscar receita realizada nos ultimos meses para comparacao
    const mesAnteriorInicio = new Date(now.getFullYear(), now.getMonth() - numMeses, 1);
    const { data: comissoesRealizadas } = await supabase_1.supabase
        .from('comissoes')
    ...
    comissoesRealizadas?.forEach((c) => {
        const data = new Date(c.data_receita);
        const key = `${data.getFullYear()}-${String(data.getMonth() + 1).padStart(2, '0')}`;
        realizadoPorMes[key] = (realizadoPorMes[key] || 0) + (c.valor_liquido || 0);
    });
    // 6. Calcular totais gerais
    const projecaoArray = Object.values(projecaoPorMes);
    const totalRenovacoes = projecaoArray.reduce((acc, m) => acc + m.renovacoes, 0);
    const totalComissaoEsperada = projecaoArray.reduce((acc, m) => acc + m.comissaoEsperada, 0);
    const totalComissoesPendentes = projecaoArray.reduce((acc, m) => acc + m.comissoesPendentes, 0);
    const totalProjetado = projecaoArray.reduce((acc, m) => acc + m.totalProjetado, 0);
    const mediaRealizadoMensal = Object.values(realizadoPorMes).reduce((acc, v) => acc + v, 0) / Math.max(Object.keys(realizadoPorMes).length, 1);
    res.json({
        ...
        totais: {
            ...
            comissaoEsperadaRenovacoes: Math.round(totalComissaoEsperada * 100) / 100,
