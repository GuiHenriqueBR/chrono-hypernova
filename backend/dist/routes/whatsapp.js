...
router.post('/webhook', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    logger_1.logger.info('Webhook WhatsApp recebido:', { event, instance });
    if (event === 'messages.upsert') {
        ...
        let { data: conversa } = await supabase_1.supabase
        ...
        if (!conversa) {
            ...
            const { data: cliente } = await supabase_1.supabase
                ...
                .single();
            const { data: novaConversa } = await supabase_1.supabase
                .from('whatsapp_conversas')
                .insert({
                telefone,
            ...
            await supabase_1.supabase
                ...
                ultima_mensagem: messageText,
                ultima_mensagem_data: new Date().toISOString(),
                nao_lidas: isFromMe ? conversa.nao_lidas : (conversa.nao_lidas || 0) + 1
            })
                .eq('id', conversa.id);
        }
        // Salvar mensagem
        await supabase_1.supabase
            ...
            tipo: message.message?.imageMessage ? 'imagem' :
                message.message?.audioMessage ? 'audio' :
                    message.message?.documentMessage ? 'documento' : 'texto',
...
router.get('/conversas', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (search) {
        query = query.or(`nome_contato.ilike.%${search}%,telefone.ilike.%${search}%`);
    }
    const { data, error, count } = await query.range(offset, offset + Number(limit) - 1);
    if (error) {
        logger_1.logger.error('Erro ao buscar conversas:', error);
...
router.get('/conversas/:conversaId/mensagens', (0, errorHandler_1.asyncHandler)(async (req, res) => {
...
// Enviar mensagem
router.post('/enviar', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (conversaId && !telefone) {
        const { data: conversa } = await supabase_1.supabase
            .from('whatsapp_conversas')
            .select('telefone')
            .eq('id', conversaId)
            .single();
    ...
    try {
        ...
        const response = await evolutionApi.post(`/message/sendText/${EVOLUTION_INSTANCE}`, {
        ...
        logger_1.logger.error('Erro ao enviar mensagem:', error);
        ...
        res.json({
            success: true,
            warning: 'Mensagem salva localmente, sera enviada quando WhatsApp estiver conectado'
        });
    }
}));
...
router.get('/templates', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (error) {
        logger_1.logger.error('Erro ao buscar templates:', error);
        return res.status(500).json({ error: 'Erro ao buscar templates' });
    }
    res.json({ data: data || [] });
}));
// Listar categorias de templates
router.get('/templates/categorias', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const categorias = [
        { value: 'saudacao', label: 'Saudacao', icon: 'hand-wave', cor: '#10b981' },
        { value: 'cotacao', label: 'Cotacao', icon: 'file-text', cor: '#8b5cf6' },
        { value: 'sinistro', label: 'Sinistro', icon: 'alert-triangle', cor: '#ef4444' },
        { value: 'cobranca', label: 'Cobranca', icon: 'dollar-sign', cor: '#f59e0b' },
        { value: 'renovacao', label: 'Renovacao', icon: 'refresh-cw', cor: '#06b6d4' },
        { value: 'aniversario', label: 'Aniversario', icon: 'gift', cor: '#ec4899' },
        { value: 'agradecimento', label: 'Agradecimento', icon: 'heart', cor: '#f43f5e' },
        { value: 'informativo', label: 'Informativo', icon: 'info', cor: '#3b82f6' },
        { value: 'outros', label: 'Outros', icon: 'message-square', cor: '#6b7280' },
    ];
    res.json({ data: categorias });
}));
// Listar variaveis disponiveis para templates
router.get('/templates/variaveis', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const variaveis = [
        { nome: '{{nome}}', descricao: 'Nome do cliente', exemplo: 'Joao Silva' },
        { nome: '{{primeiro_nome}}', descricao: 'Primeiro nome do cliente', exemplo: 'Joao' },
        { nome: '{{valor}}', descricao: 'Valor monetario', exemplo: 'R$ 1.500,00' },
        { nome: '{{data_vencimento}}', descricao: 'Data de vencimento', exemplo: '15/01/2026' },
        { nome: '{{numero_apolice}}', descricao: 'Numero da apolice', exemplo: 'APL-2025-001234' },
        { nome: '{{seguradora}}', descricao: 'Nome da seguradora', exemplo: 'Porto Seguro' },
        { nome: '{{ramo}}', descricao: 'Ramo do seguro', exemplo: 'Auto' },
        { nome: '{{parcela}}', descricao: 'Numero da parcela', exemplo: '3/12' },
        { nome: '{{protocolo}}', descricao: 'Numero do protocolo', exemplo: 'SIN-2025-0001' },
        { nome: '{{data_hoje}}', descricao: 'Data de hoje', exemplo: new Date().toLocaleDateString('pt-BR') },
        { nome: '{{hora}}', descricao: 'Hora atual', exemplo: new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }) },
        { nome: '{{corretor}}', descricao: 'Nome do corretor', exemplo: 'Maria Santos' },
    ];
    res.json({ data: variaveis });
}));
// Incrementar contador de uso do template
router.post('/templates/:id/usar', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const { error } = await supabase_1.supabase
        .from('whatsapp_templates')
        .update({
        uso_count: supabase_1.supabase.rpc('increment_uso_count', { template_id: id }),
...
router.post('/templates/processar', (0, errorHandler_1.asyncHandler)(async (req, res) => {
...
router.post('/templates', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!nome || !conteudo) {
        return res.status(400).json({ error: 'Nome e conteudo sao obrigatorios' });
    }
    if (id) {
...
// Deletar template
router.delete('/templates/:id', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const { error } = await supabase_1.supabase
        .from('whatsapp_templates')
        .delete()
...
router.post('/conversas/:conversaId/vincular', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { conversaId } = req.params;
...
router.patch('/conversas/:conversaId/status', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { conversaId } = req.params;
...
router.get('/metricas', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    // Total de conversas
    const { count: totalConversas } = await supabase_1.supabase
        .from('whatsapp_conversas')
        .select('*', { count: 'exact', head: true });
    // Conversas abertas
    const { count: conversasAbertas } = await supabase_1.supabase
        .from('whatsapp_conversas')
        .select('*', { count: 'exact', head: true })
        .in('status', ['aberta', 'em_atendimento']);
    // Mensagens hoje
    const { count: mensagensHoje } = await supabase_1.supabase
        .from('whatsapp_mensagens')
        .select('*', { count: 'exact', head: true })
        .gte('timestamp', hoje.toISOString());
    // Mensagens na semana
    const { count: mensagensSemana } = await supabase_1.supabase
        .from('whatsapp_mensagens')
        .select('*', { count: 'exact', head: true })
        .gte('timestamp', inicioSemana.toISOString());
    // Mensagens no mes
    const { count: mensagensMes } = await supabase_1.supabase
        .from('whatsapp_mensagens')
        .select('*', { count: 'exact', head: true })
        .gte('timestamp', inicioMes.toISOString());
    // Clientes vinculados
    const { count: clientesVinculados } = await supabase_1.supabase
        .from('whatsapp_conversas')
        .select('*', { count: 'exact', head: true })
    ...
    res.json({
        ...
        taxaVinculacao: totalConversas ? Math.round((clientesVinculados || 0) / totalConversas * 100) : 0
    });
}));
// Status da conexao WhatsApp
router.get('/status', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        const response = await evolutionApi.get(`/instance/connectionState/${EVOLUTION_INSTANCE}`);
        res.json({
...
router.post('/instancia/criar', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        const existingInstance = checkResponse.data?.find((inst) => inst.instance?.instanceName === instanceName);
        if (existingInstance) {
            return res.json({
                success: true,
        ...
        logger_1.logger.error('Erro ao criar instancia:', error.response?.data || error.message);
        res.status(500).json({
...
router.get('/qrcode', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        ...
        logger_1.logger.error('Erro ao obter QR Code:', error.response?.data || error.message);
        ...
        if (error.response?.status === 404 || error.response?.data?.error?.includes('not found')) {
            try {
                ...
                const qrResponse = await evolutionApi.get(`/instance/connect/${EVOLUTION_INSTANCE}`);
                return res.json({
                    success: true,
                    conectado: false,
                ...
                logger_1.logger.error('Erro ao criar instancia:', createError.response?.data || createError.message);
            }
        }
        res.json({
            success: false,
...
router.post('/desconectar', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        ...
        logger_1.logger.error('Erro ao desconectar:', error.response?.data || error.message);
        res.status(500).json({
...
// Reiniciar instancia
router.post('/reiniciar', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        ...
        logger_1.logger.error('Erro ao reiniciar:', error.response?.data || error.message);
        res.status(500).json({
...
router.get('/info', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        ...
        const instance = response.data?.find((inst) => inst.instance?.instanceName === EVOLUTION_INSTANCE);
        if (!instance) {
            return res.json({
                success: false,
                instancia: EVOLUTION_INSTANCE,
        ...
        logger_1.logger.error('Erro ao buscar info:', error.response?.data || error.message);
        res.json({
            success: false,
            instancia: EVOLUTION_INSTANCE,
...
router.post('/webhook/configurar', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { webhookUrl } = req.body;
    const url = webhookUrl || `${process.env.BACKEND_URL || 'http://localhost:3001'}/api/whatsapp/webhook`;
    try {
        ...
        logger_1.logger.error('Erro ao configurar webhook:', error.response?.data || error.message);
        res.status(500).json({
...
router.post('/enviar-midia', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { conversaId, telefone, tipo, mediaUrl, caption, fileName } = req.body;
    ...
    try {
        ...
        switch (tipo) {
            case 'imagem':
                ...
                payload = { ...payload, mediatype: 'image', media: mediaUrl, caption };
                break;
            case 'documento':
                endpoint = `/message/sendMedia/${EVOLUTION_INSTANCE}`;
                payload = { ...payload, mediatype: 'document', media: mediaUrl, fileName };
                break;
            case 'audio':
        ...
        let { data: conversa } = await supabase_1.supabase
            ...
            .single();
        if (conversa) {
            await supabase_1.supabase.from('whatsapp_mensagens').insert({
                conversa_id: conversa.id,
                message_id: response.data?.key?.id || `local-${Date.now()}`,
                direcao: 'enviada',
                tipo: tipo,
        ...
        logger_1.logger.error('Erro ao enviar midia:', error.response?.data || error.message);
        res.status(500).json({
