...
router.get('/', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (tipo) query = query.eq('tipo_financiamento', tipo);
...
router.get('/:id', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (error) {
    logger.error('Erro ao buscar financiamento:', error);
    throw error;
  }
...
router.post('/', asyncHandler(async (req: Request, res: Response) => {
  ...
  const { data: novoFinanciamento, error } = await supabase
    .from('financiamentos')
  ...
  if (parcelas_geradas && financiamento.prazo_meses && financiamento.valor_parcela) {
    ...
    await supabase.from('financiamento_parcelas').insert(parcelas);
  }

  // Registrar no histórico
  await supabase.from('financiamento_historico').insert({
    financiamento_id: novoFinanciamento.id,
    usuario_id: userId,
    tipo_evento: 'criacao',
...
router.put('/:id', asyncHandler(async (req: Request, res: Response) => {
  ...
  const { data: financiamentoAnterior } = await supabase
    .from('financiamentos')
    .select('*')
    .eq('id', id)
    .single();
...
router.get('/:id/parcelas', asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
router.post('/:id/parcelas', asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  const userId = (req as any).user?.id;
...
router.patch('/:id/parcelas/:parcelaId', asyncHandler(async (req: Request, res: Response) => {
...
router.post('/:id/amortizacao', asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  const userId = (req as any).user?.id;
  const { valor_amortizado, tipo_amortizacao, data_amortizacao, observacao } = req.body;

  // Buscar financiamento atual
  const { data: financiamento } = await supabase
    .from('financiamentos')
    .select('*')
    .eq('id', id)
    .single();
...
router.post('/:id/refinanciamento', asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  const userId = (req as any).user?.id;
...
router.get('/stats/summary', asyncHandler(async (req: Request, res: Response) => {
  ...
  const { count: quitados } = await supabase
    .from('financiamentos')
    .select('*', { count: 'exact', head: true })
    .eq('status', 'quitado');
  ...
  const { count: parcelasVencendo } = await supabase
    .from('financiamento_parcelas')
    .select('*', { count: 'exact', head: true })
    .eq('status', 'pendente')
