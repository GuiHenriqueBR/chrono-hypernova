...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { data: apolicesVencendo } = await supabase
      .from("apolices")
      .select(
        "id, numero_apolice, seguradora, ramo, data_vencimento, valor_premio, clientes(id, nome)"
      )
      .eq("status", "vigente")
    ...
    const { data: cotacoesFollowUp } = await supabase
      .from("cotacoes")
      .select(
        "id, ramo, dados_cotacao, data_envio, proximo_contato, clientes(id, nome, telefone)"
      )
    ...
    const { data: sinistrosPendentes } = await supabase
      .from("sinistros")
      .select(
        "id, numero_sinistro, status, descricao_ocorrencia, clientes(nome)"
      )
    ...
    // Apólices vencendo
    apolicesVencendo?.forEach((a: any) => {
    ...
    cotacoesFollowUp?.forEach((c: any) => {
      itens.push({
        id: `cotacao-${c.id}`,
        tipo: "follow_up",
        titulo: `Follow-up: ${
          c.dados_cotacao?.modelo || c.ramo?.toUpperCase()
        }`,
    ...
    // Sinistros pendentes
    sinistrosPendentes?.forEach((s: any) => {
      itens.push({
        id: `sinistro-${s.id}`,
        tipo: "sinistro",
    ...
    itens.sort(
      (a, b) =>
        (ordemUrgencia[a.urgencia] || 2) - (ordemUrgencia[b.urgencia] || 2)
    );
...
function calcularDiasParado(cotacao: any): number {
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { data: cotacoes, count: totalCotacoes } = await supabase
      .from("cotacoes")
    ...
    const valorPorStatus: Record<string, number> = {};
    ...
    // Cotações por ramo
    const porRamo: Record<string, { total: number; ganhas: number }> = {};
    cotacoes?.forEach((c: any) => {
      const ramo = c.ramo || "outros";
      if (!porRamo[ramo]) porRamo[ramo] = { total: 0, ganhas: 0 };
      porRamo[ramo].total++;
      if (c.status_pipeline === "fechada_ganha") porRamo[ramo].ganhas++;
    });
    ...
    res.json({
      ...
      taxas: {
        conversaoGeral:
          finalizadas > 0 ? Math.round((ganhas / finalizadas) * 100) : 0,
        envioProposta: totalCotacoes
          ...
          : 0,
        fechamento:
          (porStatus["enviada"] || 0) + (porStatus["em_negociacao"] || 0) > 0
            ? Math.round(
                (ganhas /
                  ((porStatus["enviada"] || 0) +
                    (porStatus["em_negociacao"] || 0) +
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    // 1. Apolices por Ramo
    const { data: apolicesPorRamo } = await supabase
      .from("apolices")
      .select("ramo")
      .eq("status", "vigente");
    ...
    const totalApolices = apolicesRamoChart.reduce(
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const totalMes =
        comissoes?.reduce((acc, c) => acc + (c.valor_liquido || 0), 0) || 0;
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const totalMes =
        apolices?.reduce((acc, a) => acc + (a.valor_premio || 0), 0) || 0;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { count: apolicesVigentes } = await supabase
      .from("apolices")
      .select("*", { count: "exact", head: true })
      .eq("status", "vigente");
    ...
    const { count: apolicesVencendo } = await supabase
      .from("apolices")
      .select("*", { count: "exact", head: true })
      .eq("status", "vigente")
    ...
    // Sinistros
    const { count: totalSinistros } = await supabase
      .from("sinistros")
      .select("*", { count: "exact", head: true });
    ...
    const { count: sinistrosPagos } = await supabase
      .from("sinistros")
      .select("*", { count: "exact", head: true })
      .eq("status", "pago");
    ...
    const premioTotalVigente =
      premios?.reduce((acc, a) => acc + (a.valor_premio || 0), 0) || 0;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { data: apolices } = await supabase
      .from("apolices")
      .select(
        "id, numero_apolice, ramo, seguradora, data_vencimento, valor_premio, clientes!inner(id, nome)"
      )
      .eq("status", "vigente")
      .gte("data_vencimento", hoje)
      .lte("data_vencimento", thirtyDaysFromNow.toISOString().split("T")[0])
