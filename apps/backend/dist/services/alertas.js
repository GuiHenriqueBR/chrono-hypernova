...
exports.buscarAlertas = buscarAlertas;
exports.marcarComoLido = marcarComoLido;
exports.marcarTodosComoLidos = marcarTodosComoLidos;
exports.verificarRenovacoesApolices = verificarRenovacoesApolices;
exports.verificarTarefasAtrasadas = verificarTarefasAtrasadas;
exports.verificarSinistrosPendentes = verificarSinistrosPendentes;
exports.verificarComissoesPendentes = verificarComissoesPendentes;
exports.verificarParcelasConsorcio = verificarParcelasConsorcio;
exports.verificarParcelasFinanciamento = verificarParcelasFinanciamento;
exports.verificarAniversariosClientes = verificarAniversariosClientes;
exports.gerarResumoDiario = gerarResumoDiario;
exports.executarVerificacoesAlertas = executarVerificacoesAlertas;
exports.limparAlertasAntigos = limparAlertasAntigos;
...
async function criarAlerta(alerta) {
    try {
        const { data, error } = await supabase_1.supabase
            ...
            .single();
        if (error) {
            logger_1.logger.error('Erro ao criar alerta:', error);
            return null;
        }
        return data;
    }
    catch (err) {
        logger_1.logger.error('Erro ao criar alerta:', err);
        return null;
    }
}
// Buscar alertas do usuario
async function buscarAlertas(usuarioId, options = {}) {
    try {
        ...
        logger_1.logger.error('Erro ao buscar alertas:', err);
        return [];
    }
}
// Marcar alerta como lido
async function marcarComoLido(alertaId, usuarioId) {
    try {
        const { error } = await supabase_1.supabase
            ...
            .eq('usuario_id', usuarioId);
        if (error) {
            logger_1.logger.error('Erro ao marcar alerta como lido:', error);
            return false;
        }
        return true;
    }
    catch (err) {
        logger_1.logger.error('Erro ao marcar alerta como lido:', err);
        return false;
    }
}
// Marcar todos como lidos
async function marcarTodosComoLidos(usuarioId) {
    try {
        const { error } = await supabase_1.supabase
            .from('alertas')
            .update({ lido: true })
            ...
            .eq('lido', false);
        if (error) {
            logger_1.logger.error('Erro ao marcar alertas como lidos:', error);
            return false;
        }
        return true;
    }
    catch (err) {
        logger_1.logger.error('Erro ao marcar alertas como lidos:', err);
        return false;
    }
}
// ==========================================
...
async function verificarRenovacoesApolices(diasAntecedencia = 30) {
    try {
        const dataLimite = new Date();
        dataLimite.setDate(dataLimite.getDate() + diasAntecedencia);
        const { data: apolices, error } = await supabase_1.supabase
            .from('apolices')
        ...
        if (error || !apolices) {
            logger_1.logger.error('Erro ao buscar apolices para renovacao:', error);
            return 0;
        }
        ...
        for (const apolice of apolices) {
            ...
            const { data: alertaExistente } = await supabase_1.supabase
                .from('alertas')
                .select('id')
                ...
                .single();
            if (!alertaExistente) {
                const diasParaVencer = Math.ceil((new Date(apolice.data_vencimento).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
                const prioridade = diasParaVencer <= 7 ? 'urgente' :
        ...
        logger_1.logger.info(`Alertas de renovacao criados: ${alertasCriados}`);
        return alertasCriados;
    }
    catch (err) {
        logger_1.logger.error('Erro ao verificar renovacoes:', err);
        return 0;
    }
}
// Verificar tarefas atrasadas
async function verificarTarefasAtrasadas() {
    try {
        const hoje = new Date().toISOString().split('T')[0];
        const { data: tarefas, error } = await supabase_1.supabase
            .from('tarefas')
            .select('*, clientes(nome)')
            .eq('concluida', false)
            .lt('data_vencimento', hoje);
        if (error || !tarefas) {
            logger_1.logger.error('Erro ao buscar tarefas atrasadas:', error);
            return 0;
        }
        let alertasCriados = 0;
        for (const tarefa of tarefas) {
            const { data: alertaExistente } = await supabase_1.supabase
                ...
                .single();
            if (!alertaExistente) {
                const diasAtraso = Math.ceil((new Date().getTime() - new Date(tarefa.data_vencimento).getTime()) / (1000 * 60 * 60 * 24));
                await criarAlerta({
        ...
        logger_1.logger.info(`Alertas de tarefas atrasadas criados: ${alertasCriados}`);
        return alertasCriados;
    }
    catch (err) {
        logger_1.logger.error('Erro ao verificar tarefas atrasadas:', err);
        return 0;
    }
}
// Verificar sinistros pendentes ha muito tempo
async function verificarSinistrosPendentes(diasLimite = 30) {
    try {
        const dataLimite = new Date();
        dataLimite.setDate(dataLimite.getDate() - diasLimite);
        const { data: sinistros, error } = await supabase_1.supabase
            .from('sinistros')
            .select('*, clientes(nome, usuario_id), apolices(numero_apolice)')
            .not('status', 'in', '(pago,recusado)')
            .lte('created_at', dataLimite.toISOString());
        if (error || !sinistros) {
            logger_1.logger.error('Erro ao buscar sinistros pendentes:', error);
            return 0;
        }
        let alertasCriados = 0;
        for (const sinistro of sinistros) {
            ...
            const { data: alertaExistente } = await supabase_1.supabase
                ...
                .single();
            if (!alertaExistente) {
                const diasAberto = Math.ceil((new Date().getTime() - new Date(sinistro.created_at).getTime()) / (1000 * 60 * 60 * 24));
                await criarAlerta({
                    usuario_id: usuarioId,
        ...
        logger_1.logger.info(`Alertas de sinistros pendentes criados: ${alertasCriados}`);
        return alertasCriados;
    }
    catch (err) {
        logger_1.logger.error('Erro ao verificar sinistros pendentes:', err);
        return 0;
    }
}
// Verificar comissoes pendentes
async function verificarComissoesPendentes() {
    try {
        ...
        if (error || !comissoes) {
            logger_1.logger.error('Erro ao buscar comissoes pendentes:', error);
            return 0;
        }
        // Agrupar por usuario
        const comissoesPorUsuario = {};
        ...
        logger_1.logger.info(`Alertas de comissoes pendentes criados: ${alertasCriados}`);
        return alertasCriados;
    }
    catch (err) {
        logger_1.logger.error('Erro ao verificar comissoes pendentes:', err);
        return 0;
    }
}
// Verificar parcelas de consorcio vencendo
async function verificarParcelasConsorcio() {
    try {
        const dataLimite = new Date();
        dataLimite.setDate(dataLimite.getDate() + 7);
        const hoje = new Date().toISOString().split('T')[0];
        const { data: parcelas, error } = await supabase_1.supabase
        ...
        if (error || !parcelas) {
            logger_1.logger.error('Erro ao buscar parcelas de consorcio:', error);
            return 0;
        }
        let alertasCriados = 0;
        for (const parcela of parcelas) {
            if (!parcela.consorcios?.usuario_id)
                continue;
            const { data: alertaExistente } = await supabase_1.supabase
                .from('alertas')
                .select('id')
                ...
                .single();
            if (!alertaExistente) {
                const diasParaVencer = Math.ceil((new Date(parcela.data_vencimento).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
                await criarAlerta({
        ...
        logger_1.logger.info(`Alertas de parcelas de consorcio criados: ${alertasCriados}`);
        return alertasCriados;
    }
    catch (err) {
        logger_1.logger.error('Erro ao verificar parcelas de consorcio:', err);
        return 0;
    }
}
// Verificar parcelas de financiamento vencendo
async function verificarParcelasFinanciamento() {
    try {
        const dataLimite = new Date();
        dataLimite.setDate(dataLimite.getDate() + 7);
        const hoje = new Date().toISOString().split('T')[0];
        const { data: parcelas, error } = await supabase_1.supabase
            ...
            .gte('data_vencimento', hoje);
        if (error || !parcelas) {
            logger_1.logger.error('Erro ao buscar parcelas de financiamento:', error);
            return 0;
        }
        let alertasCriados = 0;
        for (const parcela of parcelas) {
            if (!parcela.financiamentos?.usuario_id)
                continue;
            const { data: alertaExistente } = await supabase_1.supabase
                .from('alertas')
                .select('id')
        ...
        logger_1.logger.info(`Alertas de parcelas de financiamento criados: ${alertasCriados}`);
        return alertasCriados;
    }
    catch (err) {
        logger_1.logger.error('Erro ao verificar parcelas de financiamento:', err);
        return 0;
    }
}
// Verificar aniversarios de clientes (proximos 7 dias)
async function verificarAniversariosClientes() {
    try {
        const hoje = new Date();
        const diaAtual = hoje.getDate();
        const mesAtual = hoje.getMonth() + 1;
        // Buscar clientes com data de nascimento
        const { data: clientes, error } = await supabase_1.supabase
            .from('clientes')
            .select('*')
            .not('data_nascimento', 'is', null);
        if (error || !clientes) {
            logger_1.logger.error('Erro ao buscar clientes para aniversario:', error);
            return 0;
        }
        let alertasCriados = 0;
        for (const cliente of clientes) {
            if (!cliente.data_nascimento || !cliente.usuario_id)
                continue;
        ...
        logger_1.logger.info(`Alertas de aniversario criados: ${alertasCriados}`);
        return alertasCriados;
    }
    catch (err) {
        logger_1.logger.error('Erro ao verificar aniversarios:', err);
        return 0;
    }
}
// ==========================================
// GERAR RESUMO DIARIO
// ==========================================
async function gerarResumoDiario(usuarioId) {
    try {
        ...
        logger_1.logger.error('Erro ao gerar resumo diario:', err);
        return {
            urgentes: 0,
            alta_prioridade: 0,
            media_prioridade: 0,
...
// ==========================================
async function executarVerificacoesAlertas() {
    logger_1.logger.info('Iniciando verificacoes de alertas...');
    const [renovacoes, tarefas, sinistros, comissoes, consorcios, financiamentos, aniversarios] = await Promise.all([
    ...
    logger_1.logger.info('Verificacoes de alertas concluidas:', resultado);
    return resultado;
}
...
async function limparAlertasAntigos() {
    try {
        ...
        const { data, error } = await supabase_1.supabase
            ...
            .select();
        if (error) {
            logger_1.logger.error('Erro ao limpar alertas antigos:', error);
            return 0;
        }
        logger_1.logger.info(`Alertas antigos removidos: ${data?.length || 0}`);
        return data?.length || 0;
    }
    catch (err) {
        logger_1.logger.error('Erro ao limpar alertas antigos:', err);
        return 0;
    }
}
