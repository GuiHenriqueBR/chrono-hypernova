...
router.post('/', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const { data: ultimoOrdem } = await supabase_1.supabase
        .from('produtos')
        .select('ordem')
    ...
    const { data: novoProduto, error } = await supabase_1.supabase
        .from('produtos')
        .insert([{
            ...produto,
            ordem: novaOrdem,
    ...
    if (campos && campos.length > 0) {
        const camposData = campos.map((c, index) => ({
            ...c,
            produto_id: novoProduto.id,
            ordem: index + 1
        }));
        await supabase_1.supabase.from('produto_campos').insert(camposData);
    }
    logger_1.logger.info(`Novo produto criado: ${novoProduto.nome} (${novoProduto.codigo})`);
...
router.delete('/:id', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const { count: apolicesCount } = await supabase_1.supabase
        .from('apolices')
...
// Reordenar produtos
router.post('/reordenar', (0, errorHandler_1.asyncHandler)(async (req, res) => {
...
router.patch('/:id/toggle', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    // Buscar estado atual
    const { data: produto } = await supabase_1.supabase
        .from('produtos')
        .select('ativo')
        .eq('id', id)
        .single();
...
router.get('/:id/campos', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const { data, error } = await supabase_1.supabase
        .from('produto_campos')
        .select('*')
...
router.post('/:id/campos', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
...
router.put('/:id/campos/:campoId', (0, errorHandler_1.asyncHandler)(async (req, res) => {
...
router.delete('/:id/campos/:campoId', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id, campoId } = req.params;
    const { error } = await supabase_1.supabase
        .from('produto_campos')
        .delete()
...
// Reordenar campos
router.post('/:id/campos/reordenar', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
...
router.post('/seed', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user?.id;
    ...
    const { data: existentes } = await supabase_1.supabase
        .from('produtos')
        .select('codigo');
...
router.get('/categorias/list', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const categorias = [
